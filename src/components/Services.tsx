...
export default function Services() {
    return (
        <section className="w-full max-w-[90%] mx-auto my-20 md:my-[9%] flex flex-col items-center gap-10 font-raleway " >
            <div className=" text-center w-fit space-y-2 md:space-y-3 flex flex-col items-center " >
            ...
            <div className="w-full flex flex-col md:flex-row items-center md:items-stretch justify-center md:justify-between gap-7  " >
                ...
                {servicesDetails.map((card, index) => (
                    <div key={index} className="basis-1/3 max-w-[320px] md:max-w-[424px] flex flex-col items-center rounded-2xl py-10 px-[4%] gap-4 md:gap-6  bg-[#FFFFFF] border-[1px] border-[#FFFFFF] shadow-[0px_0px_20px_5px_#3F3D560D] " >
                        ...
                        <p className=" text-[#7B7A8B] text-sm lg:text-base font-normal  " > {card.description} </p>

                        <ul className="  w-full list-disc space-y-1  list-inside marker:text-[#F95C19]  " >
                            {card.possiblePackages.map((possiblePackage, index) => (
                                <li key={index} className=" text-sm lg:text-base font-normal text-[#7B7A8B] " > {possiblePackage} </li>
                            ))}
                        </ul>
