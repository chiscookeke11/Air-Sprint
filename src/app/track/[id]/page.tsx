...
export default function Page(paramsPromise: { params: Promise<{ id: string }> }) {
  ...
  // Disable scrolling when preview is shown
  useEffect(() => {
    document.body.style.overflowY = showPreview ? "hidden" : "auto";
  }, [showPreview]);

  // ✅ Fetch package purely based on tracking ID (no login required)
  useEffect(() => {
  ...
  const packageDetails = [
    { label: "Package Tracking ID:", value: currentPackage.tracking_id },
    { label: "Registered On:", value: new Date(currentPackage.created_at).toLocaleDateString() },
    { label: "Current Status:", value: currentPackage.status },
    { label: "Recipient Name:", value: currentPackage.recipient_name },
    { label: "Recipient Address:", value: currentPackage.recipient_address },
    { label: "Recipient Email:", value: currentPackage.recipient_email },
    { label: "Recipient Phone Number:", value: currentPackage.recipient_number },
    { label: "Sender Name:", value: currentPackage.sender_name },
  ];
    ...
    <div className="w-full h-fit flex items-center flex-col gap-8 md:gap-16 md:pt-14 p-4 relative">
      ...
      <div className="flex gap-[2px] items-stretch w-full max-w-[380px] font-martel">
        ...
        <Button
          onClick={trackItem}
          variant="default"
          className="px-6 py-6 md:py-[28px] rounded-sm cursor-pointer"
        >
          Send
        </Button>
      </div>
      ...
      {showPreview && (
        <ImagePreview setShowPreview={setShowPreview} imageUrl={previewUrl} />
      )}
    </div>
  );
}
